-- ============================================================
-- WezTerm config — translated from Ghostty config
-- Place at: ~/.config/wezterm/wezterm.lua
-- ============================================================
...
-- ── Cursor ───────────────────────────────────────────────────
config.default_cursor_style = "SteadyBlock"   -- cursor-style = block
config.cursor_blink_rate = 0                   -- cursor-style-blink = false
config.force_reverse_video_cursor = false
-- cursor-color = white (overridden per colorscheme below)

-- ── Mouse ────────────────────────────────────────────────────
config.hide_mouse_cursor_when_typing = true    -- mouse-hide-while-typing = true

-- ── Window ───────────────────────────────────────────────────
config.window_decorations = "NONE"             -- window-decoration = false
config.window_background_opacity = 0.8         -- background-opacity = 0.8

-- Blur — works on macOS natively; on Linux needs compositor (picom/mutter)
-- config.macos_window_background_blur = 20
-- config.win32_system_backdrop = "Acrylic"    -- Windows only

-- Balanced padding — window-padding-balance = true
...
-- window-save-state = always (restore size/position on relaunch)
...
-- ── Shell ────────────────────────────────────────────────────
config.default_prog = { "/usr/bin/nu" }        -- default shell = nushell

-- ── Rosé Pine Custom Colorscheme ─────────────────────────────
-- Translated exactly from your rosepine-custom theme file
...
local rosepine_custom = {
  ...
  brights = {
    "#6e6a86",  -- 8  bright black
    "#eb6f92",  -- 9  bright red
    "#9ccfd8",  -- 10 bright green
    "#f6c177",  -- 11 bright yellow
    "#31748f",  -- 12 bright blue
    "#c4a7e7",  -- 13 bright magenta
    "#ebbcba",  -- 14 bright cyan
    "#e0def4",  -- 15 bright white
  },

  -- Full 256-color palette from rosepine-custom
  indexed = {
  ...
  tab_bar = {
    background         = "#000000",
    active_tab         = { bg_color = "#2a2837", fg_color = "#e0def4", intensity = "Normal" },
    inactive_tab       = { bg_color = "#000000", fg_color = "#6e6a86" },
    inactive_tab_hover = { bg_color = "#26233a", fg_color = "#e0def4" },
    new_tab            = { bg_color = "#000000", fg_color = "#6e6a86" },
    new_tab_hover      = { bg_color = "#26233a", fg_color = "#e0def4" },
  },
}

config.colors = rosepine_custom
